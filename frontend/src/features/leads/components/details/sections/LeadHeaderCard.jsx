...
export default function LeadHeaderCard({ viewModel, rawData, actions, onDelete, onEdit }) {
  ...
  return (
    <section className="premium-card rounded-lg p-lg flex flex-col gap-lg">
        ...
        <div className="flex gap-lg">
          ...
          <div className="flex flex-col gap-xs">
            <div className="flex items-center gap-md">
              ...
              <span className="px-md py-xs bg-primary-fixed text-on-primary-fixed rounded-full text-label-sm font-bold">LEAD ID: {enquiryNo}</span>
              <span className="px-md py-xs bg-[#E6F4EA] text-[#137333] rounded-full text-label-sm font-bold">{qualificationStatus}</span>
            </div>
            <p className="font-body-lg text-body-lg text-on-surface-variant">{course}</p>
            <div className="flex gap-lg mt-sm text-body-sm text-on-surface-variant">
              <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-sm">mail</span> {email}</span>
              <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-sm">call</span> {phone}</span>
              <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-sm">location_on</span> {locationStr}</span>
            </div>
          </div>
        </div>
        <div className="flex gap-sm">
          {actions}
          <button onClick={onDelete} className="border-2 border-error text-error px-xl py-md rounded-full font-label-md hover:bg-error-container/20 transition-all flex items-center gap-sm">
            <span className="material-symbols-outlined text-sm">delete</span>
            Delete
          </button>
          <button onClick={onEdit} className="border border-outline px-xl py-md rounded-full font-label-md hover:bg-surface-container-high transition-all flex items-center gap-sm">
            More Actions
            <span className="material-symbols-outlined text-sm">expand_more</span>
          </button>
        </div>
      </div>
      <div className="grid grid-cols-4 gap-xl pt-lg border-t border-outline-variant">
        <div className="flex items-center gap-md">
