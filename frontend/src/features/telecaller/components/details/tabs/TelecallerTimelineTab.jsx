...
import TimelineItem from "@/features/leads/components/details/timeline/TimelineItem";
import { useTelecallerLeadTimeline } from "../../../hooks/useTelecallerLeadTimeline";
...
export default function TelecallerTimelineTab({ leadId }) {
  const { data, isLoading, isError, error } = useTelecallerLeadTimeline(leadId);
  ...
  if (isError && error?.response?.status !== 404) {
    return (
      <div className="p-8 text-center text-sm text-red-500 border border-red-100 rounded-xl bg-red-50">
        {error?.response?.data?.message || "Failed to load timeline for this lead."}
      </div>
    );
  }
